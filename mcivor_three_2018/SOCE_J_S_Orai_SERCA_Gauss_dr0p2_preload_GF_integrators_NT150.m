...
area_orai_channel=pi*(.55/2)^2; % max diameter Orai=0.55nm (Parekh 2005)
%% ER-PM micro-domain 
...
%     [zzmicro,rrmicro]=meshgrid(r_m,z_m);
    ...
    if exist('vol_int_J_z_NT150')
    disp('GF J integrators preloaded')
    else
    [vol_int_J_z_NT150, vol_int_J_rt_NT150]=create_vol_integration_matrices_J_25072017(r_a,dt,val_Dm,mu_max_m,beta_max_m,n_max_m,dr_m,dz_m,dtheta,theta_prime,r_m_prime,z_m_prime);
...
% Geometry specific to subPM ER
    ...
    if exist('vol_int_S_z_NT150')
        disp('GF S integratos preloaded')
    else
    [vol_int_S_z_NT150, vol_int_S_rt_NT150]=create_vol_integration_matrices_S_10082017(r_a,dt,val_De,mu_max_s,beta_max_s,n_max_s,dr_s,dz_s,dtheta,theta_prime,r_s_prime,z_s_prime,L1);
...
tic
    ...
    fn_particular_soln=['Boundary_solution_z_H_junction-mu_max_',num2str(mu_max_m),'-beta_max_',num2str(beta_max_m),'-n_max_',num2str(n_max_m)...
    ...
    J_CRAC_flux_int=volume_integral_junction(vol_int_J_z_NT150,vol_int_J_rt_NT150,0,flux_CRAC_M);
